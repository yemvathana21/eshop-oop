...
class CheckoutController extends Controller {
    ...
    public function index() {
        ...
        $this->render('customer/checkout', [
            'title' => 'Checkout - General Online Store',
            'cart' => $cart,
            'subtotal' => $subtotal,
            'addresses' => $addresses,
    ...
    public function process() {
        ...
        if (!empty($address['province_code'])) {
            $p = (new \App\Models\Location\Province())->findByCode($address['province_code']);
            $province = $p ? $p['name_en'] : '';
        }
        if (!empty($address['district_code'])) {
            $d = (new \App\Models\Location\District())->findByCode($address['district_code']);
            $district = $d ? $d['name_en'] : '';
        }
        if (!empty($address['commune_code'])) {
            $c = (new \App\Models\Location\Commune())->findByCode($address['commune_code']);
            $commune = $c ? $c['name_en'] : '';
        }
        if (!empty($address['village_code'])) {
            $v = (new \App\Models\Location\Village())->findByCode($address['village_code']);
            $village = $v ? $v['name_en'] : '';
        }
    ...
    public function orderConfirmation() {
        ...
        $this->render('customer/order_confirmation', [
            'title' => 'Order Placed - General Online Store',
            'order' => $order,
            'items' => $items,
        ]);
    }
    ...
    public function myOrders() {
        ...
        $this->render('customer/my_orders', [
            'title' => 'My Orders - General Online Store',
            'orders' => $enriched
        ]);
    }
}
