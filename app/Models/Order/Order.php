...
class Order {
    ...
    public function createOrder($userId, $totalPrice, $cartItems, $options = []) {
        try {
            ...
            $shippingPhone = $options['shipping_phone'] ?? null;
            ...
            $stmt = $this->db->prepare("INSERT INTO orders (user_id, total_price, status, invoice_number, shipping_name, shipping_phone, shipping_address, shipping_method, shipping_cost, payment_method) VALUES (?, ?, 'pending', ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$userId, $totalPrice, $invoiceNumber, $shippingName, $shippingPhone, $shippingAddress, $shippingMethod, $shippingCost, $paymentMethod]);
