<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            <h2 id="formTitle" class="text-lg font-bold text-gray-900 dark:text-white mb-4">Add End Category</h2>
            <form action="<?= BASE_URL ?>admin/end-category/save" method="POST" id="endCatForm">
                <input type="hidden" name="id" id="catId">
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Mid Level Category *</label>
                        <select name="parent_id" id="catParentId" required
                            ...
                            <?php foreach ($midCategories as $m): ?>
                            <option value="<?= $m['id'] ?>"><?= htmlspecialchars($m['name']) ?> (<?= htmlspecialchars($m['parent_name']) ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-700 border-b border-gray-100 dark:border-gray-600 text-left">
                        ...
                        <th class="py-3 px-4 font-semibold text-gray-600 dark:text-gray-300">End Category</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr>
                        <td colspan="5" class="py-10 text-center text-gray-500">No categories found.</td>
                    </tr>
                    <?php else: ?>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            ...
                            <td class="py-3 px-4"><span class="text-xs bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 px-2 py-0.5 rounded-full"><?= htmlspecialchars($c['mid_name']) ?></span></td>
                            <td class="py-3 px-4"><span class="text-xs bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-full"><?= htmlspecialchars($c['top_name']) ?></span></td>
                            <td class="py-3 px-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <a href="<?= BASE_URL ?>admin/end-category/delete?id=<?= $c['id'] ?>" onclick="return confirm('Are you sure?')" class="text-red-500 hover:text-red-700 p-1">
...
function editCat(id, name, slug, parentId) {
    document.getElementById('formTitle').textContent = 'Edit End Category';
...
function resetForm() {
    document.getElementById('formTitle').textContent = 'Add End Category';
    document.getElementById('catId').value = '';
    document.getElementById('endCatForm').reset();
