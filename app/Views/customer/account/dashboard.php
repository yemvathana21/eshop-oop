...
$themeOptions = [
    ['id' => 'light', 'label' => 'Light', 'desc' => 'Clean white background', 'icon' => 'fa-sun'],
    ['id' => 'dark', 'label' => 'Dark', 'desc' => 'Standard dark mode', 'icon' => 'fa-moon'],
    ['id' => 'amoled', 'label' => 'AMOLED', 'desc' => 'Pure black for OLED', 'icon' => 'fa-moon'],
    ['id' => 'system', 'label' => 'System', 'desc' => 'Match your device', 'icon' => 'fa-desktop'],
];
...
<div class="max-w-2xl mx-auto space-y-6 pb-10">
    ...
    <!-- Theme -->
    <div>
        <h3 class="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 px-1 mb-1">Mini App</h3>
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
            <div onclick="openModal('theme')" class="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700" data-search="theme">
                <div class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-sm text-gray-500 capitalize"><?= $savedTheme ?></span>
                    <i class="fas fa-chevron-right text-xs text-gray-400"></i>
                </div>
            </div>
        </div>
    ...
    <!-- Privacy -->
    <div>
        <h3 class="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 px-1 mb-1">Privacy & Security</h3>
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
    ...
    <div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <div onclick="openModal('billing')" class="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700" data-search="billing address">
                <div class="flex items-center gap-3">
    ...
    <!-- Quick Links -->
    <div>
        <h3 class="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 px-1 mb-1">More</h3>
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <a href="<?= BASE_URL ?>account/addresses" class="flex items-center justify-between px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-sm font-medium text-gray-900 dark:text-white">Addresses</span>
                </div>
                <i class="fas fa-chevron-right text-xs text-gray-400"></i>
            </a>
            <a href="<?= BASE_URL ?>account/payment-methods" class="flex items-center justify-between px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700">
                <div class="flex items-center gap-3">
...
<!-- Modal Overlay -->
<div id="modalOverlay" class="fixed inset-0 z-50 hidden items-center justify-center p-4" style="background: rgba(0,0,0,0.4);" onclick="if(event.target===this)closeModal()">
    <div id="modalSheet" class="w-full sm:max-w-md bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl shadow-xl max-h-[90vh] flex flex-col overflow-hidden">
    ...
    html: '<form id="profileForm" onsubmit="return submitProfileForm(event)">' +
        '<div class="flex flex-col items-center py-4 mb-4">' +
            '<div class="relative mb-2">' +
                ...
                '<?php else: ?>' +
                '<div id="profilePreview" class="w-20 h-20 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-2xl font-bold text-blue-600 dark:text-blue-400 border-2 border-blue-500"><?= strtoupper(substr($user['name'], 0, 1)) ?></div>' +
                '<?php endif; ?>' +
        ...
        '</div>' +
        '<div class="space-y-4">' +
            '<div><label class="block text-xs font-medium text-gray-500 mb-1">Full Name</label><input type="text" name="name" value="<?= htmlspecialchars($user['name']) ?>" required class="w-full px-3.5 py-2.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-sm text-gray-900 dark:text-white outline-none focus:border-blue-500"></div>' +
...
modalContents.username = {
    ...
    html: '<form id="usernameForm" onsubmit="return submitUsernameForm(event)">' +
        ...
        '<div><label class="block text-xs font-medium text-gray-500 mb-1">Handle</label><input type="text" name="username" value="<?= htmlspecialchars($user['username'] ?? '@user') ?>" required class="w-full px-3.5 py-2.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-sm text-gray-900 dark:text-white outline-none focus:border-blue-500"></div>' +
        '<div class="flex gap-3 mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">' +
            '<button type="button" onclick="closeModal()" class="flex-1 py-2.5 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 transition">Cancel</button>' +
            '<button type="submit" class="flex-1 py-2.5 rounded-lg text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 transition">Save</button>' +
        '</div>' +
    ...
    html: '<div>' +
        ...
        '<div class="space-y-2">' +
            ...
            '<div class="flex items-center justify-between p-3 rounded-lg border border-gray-200 dark:border-gray-700">' +
                ...
                '</div>' +
                '<button onclick="toggleConnected(\'<?= $prov['id'] ?>\')" class="px-3 py-1.5 rounded-lg text-xs font-medium ' + (accountConnected('<?= $prov['id'] ?>', <?= json_encode($connectedAccounts) ?>) ? 'bg-green-50 dark:bg-green-900/20 text-green-600' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-600') + '">' +
...
modalContents.theme = {
    title: 'Theme',
    html: '<div>' +
        '<p class="text-sm text-gray-500 mb-4">Choose your preferred appearance.</p>' +
        '<div class="space-y-2">' +
            '<?php foreach ($themeOptions as $t): ?>' +
            '<div onclick="selectTheme(\'<?= $t['id'] ?>\')" class="flex items-center justify-between p-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition">' +
                '<div class="flex items-center gap-3">' +
                    '<div class="w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-600 dark:text-gray-400"><i class="fas <?= $t['icon'] ?>"></i></div>' +
                    '<div><p class="text-sm font-medium text-gray-900 dark:text-white"><?= $t['label'] ?></p><p class="text-xs text-gray-500"><?= $t['desc'] ?></p></div>' +
                '</div>' +
                '<?php if ($savedTheme === $t['id']): ?><i class="fas fa-check text-blue-600"></i><?php endif; ?>' +
            '</div>' +
            '<?php endforeach; ?>' +
        '</div>' +
    '</div>'
...
function selectTheme(theme) {
    fetch('<?= BASE_URL ?>account/preference/save', {
        ...
        closeModal();
    }).catch(function() { showToast('Theme saved'); closeModal(); });
}
